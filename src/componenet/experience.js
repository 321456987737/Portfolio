...
export default function DynamicPortfolioScroller() {
  ...
  const getImageStyle = useCallback((image, imageIndex, projectIndex) => {
    ...
    // Mobile layout - show only 3 images with fixed height
    if (isMobile) {
      // Only show first 3 images on mobile
      if (imageIndex >= 3) {
        return { display: 'none' };
      }
      ...
      const baseStyle = {
        height: "120px",
        width: "30%", // 3 images with some gap
        position: "relative",
        top: "auto",
        left: "auto",
        right: "auto",
        ...
        transform: "none",
        margin: "0 1.5%",
        zIndex: isHovered ? 999 : 10 + imageIndex,
    ...
    const size = sizeConfig[image.size].md;
    const position = positionConfig[image.position].md;
          ...
          {projects.map((project, projectIndex) => (
            ...
            >
              ...
              <div className={`absolute inset-0 ${isMobile ? 'flex items-center justify-center' : ''}`}>
                {project.images.map((image, imageIndex) => {
                  ...
                  return (
                    <div
                      key={imageIndex}
                      className={`rounded-2xl overflow-hidden shadow-2xl transition-all duration-400 ${
                        isHovered 
                          ? 'scale-105 shadow-3xl' 
                          : 'hover:scale-102 hover:shadow-3xl'
                      } ${isMobile ? 'rounded-lg flex-shrink-0' : 'rounded-2xl absolute'}`}
              ...
              <div className={`absolute text-white z-50 ${
                isMobile 
                  ? 'left-4 right-4 bottom-20 max-w-full' // Moved up to avoid overlapping with images
                ...
                <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 z-50">
                  <div className="bg-black/60 backdrop-blur-lg rounded-full px-4 py-2 border border-white/20">
                    <span className="text-white text-sm font-medium">
                      {Math.min(3, project.images.length)} / {project.images.length} images
                    </span>
                  </div>
                </div>
              )}
